...
** Javascript just hides problems, it doesn’t eliminate them. It allows you to code without fixing your bugs. Don’t do it. If you want your code to be maintainable and scalable, write in (well-typed) Typescript.
* Loose Coupling
...
- DashboardPage/
--- components/
----- DashboardTable.tsx   👈 Wrapped Component
--- index.tsx
--- DashboardPage.tsx
....
...
src/
--- components/
------- Table.tsx    👈 Re-used, Abstracted, Wrapped Component
--- DashboardPage/
----- index.tsx
----- DashboardPage.tsx
....
...
=== Do Not Trust; Validate (and Throw)
...
Throw an error if you do not have the data you need as defined by your schemas, but simply send a warning log if there are extra attributes or attributes you don't care about do not meet your expected schema definition.

Why throw? Why not just let Javascript 'handle it'?

Because then you should just be using Javascript. If you aren't enforcing your types as contracts, then you have ceded all of the benefits of the strict type system.
...
> Always handle potential errors in async/await method calls. This may mean a lot of try/catch blocks in your data fetching code.

https://twitter.com/davidkpiano/status/1561082799515262976?s=21&t=dBvTZT-lFUtBnM4htNcBDg

=== React-Router 6.4+ Loader Pattern

> Use loaders. Use react-query client-side when not able to use loaders.

NOTE: The loader+react-query pattern can be powerful when caching is needed

=== React Query

There was a time when React Query (Tanstack Query) was the only sensible option when we were all using SPAs. Now, it's an optimization for Multi-page applications, so you probably don't need it.

==== Query Chains and Dependent Queries


> Combine dependent-queries into single query function rather than toggling `enabled` on each query as you work through them.
...
> Infrastructure as Code (IaC) should always be the default. Manual configuration is a last resort and to be avoided as it is error-prone and not review-able.
...
> Resources should contain appropriate tags. E.g. `terraform: true` (or whatever IaC tooling the project uses), `environment: <dev or prod>`, `service: <name>`, `workspace: <terraform workspace>`, etc.
...
> S3 Buckets should be managed by your IaC tooling and in general should include the following best-practices:
>
...
> Use something like `typebox` or `zod` to validate incoming `Requests` and cast them to their appropriate type *_after validating the data shape_*. Then validate your own `Response` to make sure you aren't breaking your contract.
...
const getTitles = async ({ id }:{ id: string }): Promise<string[]> => {
  ...
  return false; // 👈 New Code (though this is against our Function Purity best practice, below.)
};
----

If you do this, you have implicitly changed the return type from `Promise<string[]>` to `Promise<false | string[]>` because Typescript will recognize the new, correct return definition automatically. But Typescript will not force you to think about the consequences of that contract change or force you to handle `false` everywhere you might need to (though in many cases, TS will throw errors since `false` has no overlap with `string[]`). I f no return type was manually specified initially, you may not realize that you have broken the function’s contract, and may not realize that you now need to also refactor how it is used in every previous instance since you now need to handle whether or not it returns `false`.

If instead, the return type is specified manually as `Promise<string[]>` from the beginning, then you would have to change the return type to correct the TS error that occurred when you refactored the function, which would clue you in to the fact that you’ve made a breaking change to the function definition.
...
As described in the link:#theo-return-types[video below], overrides can lead to mismatching return types and actual data if not careful. Avoid return types when multiples types can _legitimately_ be returned correctly (though treat such code with suspicion and extra care).
...
Using a prepended _indicator_ that denotes _being_ or _having_ a condition is a legible way to indicate the type of a variable or property. Without this Best Practice, if you see a property or variable named `feature`, it could be a function (i.e. you want to send off a request to ‘feature’ this item), it could be a boolean (i.e. denoting whether it is a feature), or it could be a string or number (i.e. containing the value of the ‘feature’). To limit this confusion (and thus increase maintainability), prepend an indicator verb, such as ‘is’, ‘has’, ‘was’, ‘did’, ‘will’, etc. that _indicates_ the status of a condition: `isFeature`.
...
NOTE: See link:./documentation.adoc[the full Best Practices document].
